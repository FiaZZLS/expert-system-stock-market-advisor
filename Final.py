...
import sys
from PyQt5.QtWidgets import QApplication, QMainWindow, QAction, QPushButton, QWidget, QVBoxLayout, QLabel, QLineEdit, QHBoxLayout, QFileDialog , QSlider,QDateEdit
...
class InputPage(QWidget):
    def __init__(self, parent):
        super().__init__(parent) # Set the background color using a valid hexadecimal color code
        self.parent = parent
        ...
        self.start_date_input.setStyleSheet("font-size: 18px;")
        ...
        self.end_date_input.setStyleSheet("font-size: 18px;")
        ...
        start_button.setStyleSheet("font-size: 18px;")  # Adjust button font size
        start_button.clicked.connect(self.start_analysis)
        layout.addWidget(start_button)

        open_folder_button = QPushButton("Show Results")
        open_folder_button.setStyleSheet("font-size: 18px;")  # Adjust button font size
    ...
    def start_analysis(self):
        ...
        start_date = mpdates.date2num(start_date)
        end_date = datetime.strptime(end_date, "%m/%d/%Y")
        end_date = mpdates.date2num(end_date)
        # You can use start_date and end_date for further processing
        print("Start Date:", start_date)
        print("End Date:", end_date)
        file = dir_ +"\\Desktop\\Projet system expert" +'\\Final_OHLC.csv'

        df = pd.read_csv(file)
        start_index = df[df['Date'] == start_date].index
        end_index = df[df['Date'] == end_date].index
        print(start_index)
        print(end_index)
        ohlc = df.iloc[int(start_index[0]):int(end_index[0]), :]
        real_columns = ["Date","Open","High","Low","Close"]
...
class ImageDisplayPage(QWidget):
    def __init__(self, parent, image_paths, image_labels):
        ...
        self.label.setStyleSheet("font-size: 35px;color: #2a707c;")
